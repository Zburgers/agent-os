...
(() => {
  ...
  function renderDetail(kind, record) {
    ...
    if (kind === 'jobs') {
      return '<dl class="detail-grid"><dt>Job identifier</dt><dd>' + escape(record.id) + '</dd><dt>Current status</dt><dd>' + badge(record.status) + '</dd><dt>Retry attempts</dt><dd>' + escape(record.attempts) + ' / ' + escape(record.max_attempts) + '<span class="record-meta">Retries reset after a successful recurring run.</span></dd><dt>Next scheduled run</dt><dd>' + when(record.next_run_at) + '</dd><dt>Related work</dt><dd>' + escape(record.ticket_title || record.venture_name || 'Not linked') + '</dd><dt>Current failure</dt><dd>' + escape(record.last_error || 'None') + '</dd></dl><h3>Run history</h3><p class="page-description">Every completed and failed execution is listed below, newest first. Provider results are summarized before the technical payload.</p><div class="run-history">' + ((record.runs || []).map((run, index) => renderJobRun(run, index)).join('') || empty('No runs yet.')) + '</div><div class="actions">' + jobButtons(record) + '</div>';
    }
  ...
  function renderJobRun(run, index) {
