...
(() => {
  ...
  async function loadFinance() {
    try {
      const [records, overview, wallet] = await Promise.all([api('/api/ledger?' + params()), api('/api/overview'), api('/api/wallet/status')]);
      ...
      const walletSection = '<section id="wallets" class="section"><div class="dialog-top"><div><h2>Wallets and digital assets</h2><p>Blockchain balances remain in native atomic units and are never blended into INR accounting.</p></div><a class="button" href="/wallet">Manage wallet</a></div>' + (wallet.link ? '<div class="row"><div><strong>Ethereum Mainnet</strong><span>' + escape(wallet.link.address) + ' · ' + escape(walletBalance) + '</span><span>' + escape(wallet.intents.length) + ' recorded transaction drafts</span></div>' + badge(wallet.configured ? 'active' : 'pending') + '</div>' : empty('No wallet has been linked. Add one through the authenticated wallet control.')) + '</section>';
