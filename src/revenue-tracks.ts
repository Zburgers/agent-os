...
export class RevenueTrackService {
  ...
  async create(input: RevenueTrackInput, actor: TrackActor) {
  ...
  async update(id: string, input: RevenueTrackUpdate, actor: TrackActor) {
    ...
    if (input.confidence !== undefined) fields.push(['confidence', integer(input.confidence, 'confidence', 0)]);
    ...
    return this.mutate(id, fields, actor, 'revenue_track_updated');
  }

  async reparent(id: string, parentTrackId: string | null, actor: TrackActor) { return this.mutate(id, [['parent_track_id', parentTrackId]], actor, 'revenue_track_reparented'); }
  async archive(id: string, status: 'completed' | 'killed', actor: TrackActor) { return this.mutate(id, [['status', validateStatus(status)]], actor, 'revenue_track_archived'); }
  ...
  private async mutate(id: string, fields: Array<[string, unknown]>, actor: TrackActor, event: string) {
    const client = await this.database.connect();
    try {
      await client.query('BEGIN');
