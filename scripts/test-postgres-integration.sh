...
docker compose run --rm -T app sh -c "DATABASE_URL=\"\${DATABASE_URL%/*}/$test_database\" npm run migrate"
docker compose run --rm -T app sh -c "DATABASE_URL=\"\${DATABASE_URL%/*}/$test_database\" RUN_POSTGRES_INTEGRATION=true node --test --experimental-strip-types test/postgres-integration.test.ts test/external-effect-crash.test.ts"
