...
import { CODEX_OPERATING_BLOCK_LOCK_KEY, markManualCodexOccurrenceLockCollision } from '../src/codex-operating-block-control.ts';

const occurrenceKey = process.env.CODEX_OCCURRENCE_KEY ?? `scheduled:${new Date().toISOString().slice(0, 10)}`;
const triggerKind = process.env.CODEX_TRIGGER_KIND === 'manual' ? 'manual' : 'scheduled';
...
try {
  ...
  const lock = await lockClient.query('SELECT pg_try_advisory_xact_lock(hashtextextended($1, 0)) AS acquired', [CODEX_OPERATING_BLOCK_LOCK_KEY]);
  if (!lock.rows[0]?.acquired) {
    await lockClient.query('ROLLBACK');
    transactionOpen = false;
    lockClient.release();
    if (triggerKind === 'manual') await markManualCodexOccurrenceLockCollision(pool, occurrenceKey);
    console.log(JSON.stringify({ status: 'skipped', exitReason: 'lock_collision', threadId: CODEX_THREAD_ID }));
    await closeDatabase();
    process.exit(0);
  }
  const occurrence = await pool.query(
    `INSERT INTO codex_operating_block_occurrences(occurrence_key,intended_date,trigger_kind)
     ...
     ON CONFLICT(occurrence_key) DO UPDATE SET occurrence_key=EXCLUDED.occurrence_key
     RETURNING id,status`,
    [occurrenceKey, triggerKind],
  );
  occurrenceId = occurrence.rows[0].id;
  if (occurrence.rows[0].status !== 'queued') {
    await lockClient.query('COMMIT');
    transactionOpen = false;
    lockClient.release();
    console.log(JSON.stringify({ status: 'skipped', exitReason: 'duplicate_occurrence', threadId: CODEX_THREAD_ID }));
    await closeDatabase();
    process.exit(0);
  }
  ...
  if (occurrenceMeta.rows[0]?.trigger_kind === 'scheduled' && paused.rows[0]?.schedule_paused) {
    ...
    transactionOpen = false;
    lockClient.release();
    console.log(JSON.stringify({ status: 'skipped', exitReason: 'schedule_paused', threadId: CODEX_THREAD_ID }));
    await closeDatabase();
    process.exit(0);
  }
  ...
  if (occurrenceId) {
    try { await pool.query("UPDATE codex_operating_block_occurrences SET status='failed',finished_at=now() WHERE id=$1 AND status IN ('queued','running')", [occurrenceId]); } catch {}
  }
