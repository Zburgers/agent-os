...
The dedicated wallet remains separate from the owner wallet. Policy versions, activation, revocation, receiving, and governed withdrawals are PostgreSQL operational state, not contextual memory. Never copy private keys, raw signatures, or unbounded transaction material into memory; new policy versions begin as `draft` and owner activation/revocation remains audited.
