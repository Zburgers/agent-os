...
const active: WalletPolicy = { expiresAt: '2099-01-01T00:00:00Z', chainIds: [8453], providers: ['bountybook'], recipientAllowlist: ['0xrecipient'], contractAllowlist: ['0xcontract'], messageTypes: ['personal_sign'], selectors: ['0x12345678'], maxTransactionValueMinor: 1000, maxGasMinor: 100, dailyBudgetMinor: 5000, totalBudgetMinor: 10000 };
...
test('wallet policy denies unknown dimensions and enforces every bounded dimension', () => {
  ...
  for (const request of [{ chainId: 1 }, { provider: 'unknown' }, { recipient: '0xbad' }, { contract: '0xbad' }, { messageType: 'typed_data' }, { selector: '0xbad' }, { valueMinor: 1001 }, { gasMinor: 101 }, { dailyUsedMinor: 5000, valueMinor: 1 }, { totalUsedMinor: 10000, valueMinor: 1 }]) {
    assert.equal(evaluateWalletPolicy(active, { ...request, now: new Date('2026-08-01T00:00:00Z') }).allowed, false);
  }
  assert.equal(evaluateWalletPolicy(active, {}, { lifecycleStatus: 'draft' }).code, 'policy_not_active');
...
test('wallet budget includes the requested value and gas exposure', () => {
  assert.equal(evaluateWalletPolicy(active, { valueMinor: 1000, gasMinor: 1, dailyUsedMinor: 4000, totalUsedMinor: 8000 }).code, 'daily_budget_exceeded');
  assert.equal(evaluateWalletPolicy(active, { valueMinor: 1, gasMinor: 1, dailyUsedMinor: 4998, totalUsedMinor: 9998 }).allowed, true);
});
