...
test('jobs page exposes the latest recurring run separately from retry attempts', () => {
  ...
  for (const value of ['Last run', 'last_run_status', 'last_finished_at', 'Total runs', 'run_count', 'No runs yet', 'Run history', 'Outcome', 'Provider bid', 'Technical details', 'job-expand', 'aria-label="Expand job details"', "setAttribute('aria-labelledby', 'detailTitle')", 'job-modal-enhancements', 'dashboard-table-spacing', 'job-cell', 'job-metric', 'job-timing', 'job-last-run']) assert.ok(html.includes(value), value);
});
...
test('accounts page is an owner-facing live credential observability surface', () => {
  const html = renderControlPlane('accounts' as any, {}, 'csrf-test');
