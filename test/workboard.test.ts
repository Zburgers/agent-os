...
test('jobs page exposes the latest recurring run separately from retry attempts', () => {
  ...
  for (const value of ['Last run', 'last_run_status', 'last_finished_at', 'Total runs', 'run_count', 'No runs yet', 'Run history', 'Outcome', 'Provider bid', 'Technical details', 'job-expand', 'aria-label="Expand job details"', "setAttribute('aria-labelledby', 'detailTitle')", 'job-modal-enhancements']) assert.ok(html.includes(value), value);
});
